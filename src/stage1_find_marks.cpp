...
#ifdef __SSE4_2__
  in.lo = _mm_loadu_si128(reinterpret_cast<const __m128i *>(ptr + 0));
//  in.hi = _mm_set1_epi8(0x20);
  in.hi = _mm_loadu_si128(reinterpret_cast<const __m128i *>(ptr + 16));
#elif defined(__ARM_NEON)
#ifndef TRANSPOSE
...
really_inline void check_utf8(simd_input in,
  ...
  if ((_mm_testz_si128(_mm_or_si128(in.lo, in.hi), highbit)) == 1) {
...
void p64_hex_u8(uint64_t in) {
    alignas(64) uint8_t v[8];
    v[0] = in & 0xFF;
    v[1] = (in & 0xFF00) >> 8;
    v[2] = (in & 0xFF0000) >> 16;
    v[3] = (in & 0xFF000000) >> 24;
...
// cheaper in AVX512.
really_inline uint32_t cmp_mask_against_input(simd_input in, uint8_t m) {
#ifdef __SSE4_2__
  const __m128i mask = _mm_set1_epi8(m);
  __m128i cmp_res_0 = _mm_cmpeq_epi8(in.lo, mask);
  uint32_t res_0 = static_cast<uint32_t>(_mm_movemask_epi8(cmp_res_0));
  __m128i cmp_res_1 = _mm_cmpeq_epi8(in.hi, mask);
  uint32_t res_1 = _mm_movemask_epi8(cmp_res_1);
...
really_inline uint32_t unsigned_lteq_against_input(simd_input in, uint8_t m) {
#ifdef __SSE4_2__
  const __m128i maxval = _mm_set1_epi8(m);
  __m128i cmp_res_0 = _mm_cmpeq_epi8(_mm_max_epu8(maxval,in.lo),maxval);
  uint32_t res_0 = static_cast<uint32_t>(_mm_movemask_epi8(cmp_res_0));
  __m128i cmp_res_1 = _mm_cmpeq_epi8(_mm_max_epu8(maxval,in.hi),maxval);
  uint32_t res_1 = _mm_movemask_epi8(cmp_res_1);
  return res_0 | (res_1 << 16);
#elif defined(__ARM_NEON)
  const uint8x16_t mask = vmovq_n_u8(m);
...
really_inline uint32_t find_quote_mask_and_bits(simd_input in, uint32_t odd_ends,
    uint32_t &prev_iter_inside_quote, uint32_t &quote_bits, uint32_t &error_mask) {
  ...
  uint32_t unescaped = unsigned_lteq_against_input(in, 0x1F);
  error_mask |= quote_mask & unescaped;
  // right shift of a signed value expected to be well-defined and standard
  // compliant as of C++20,
  // John Regher from Utah U. says this is fine code
  prev_iter_inside_quote =
      static_cast<uint32_t>(static_cast<int32_t>(quote_mask) >> 31);
  return quote_mask;
}
...
really_inline void find_whitespace_and_structurals(simd_input in,
  ...
  __m128i v_lo = _mm_and_si128(
      _mm_shuffle_epi8(low_nibble_mask, in.lo),
      _mm_shuffle_epi8(high_nibble_mask,
                          _mm_and_si128(_mm_srli_epi32(in.lo, 4),
                                           _mm_set1_epi8(0x7f))));

  __m128i v_hi = _mm_and_si128(
      _mm_shuffle_epi8(low_nibble_mask, in.hi),
      _mm_shuffle_epi8(high_nibble_mask,
                          _mm_and_si128(_mm_srli_epi32(in.hi, 4),
                                           _mm_set1_epi8(0x7f))));
  __m128i tmp_lo = _mm_cmpeq_epi8(
      _mm_and_si128(v_lo, structural_shufti_mask), _mm_set1_epi8(0));
  __m128i tmp_hi = _mm_cmpeq_epi8(
      _mm_and_si128(v_hi, structural_shufti_mask), _mm_set1_epi8(0));

  uint32_t structural_res_0 =
      static_cast<uint32_t>(_mm_movemask_epi8(tmp_lo));
  uint32_t structural_res_1 = _mm_movemask_epi8(tmp_hi);
  ...
  uint32_t ws_res_0 = static_cast<uint32_t>(_mm_movemask_epi8(tmp_ws_lo));
  uint32_t ws_res_1 = _mm_movemask_epi8(tmp_ws_hi);
  whitespace = ~(ws_res_0 | (ws_res_1 << 16));
#elif defined(__ARM_NEON)
...
really_inline void flatten_bits(uint32_t *base_ptr, uint32_t &base,
  ...
  while (bits != 0u) {
    base_ptr[base + 0] = static_cast<uint32_t>(idx) - 32 + trailingzeroes(bits);
    bits = bits & (bits - 1);
    base_ptr[base + 1] = static_cast<uint32_t>(idx) - 32 + trailingzeroes(bits);
    bits = bits & (bits - 1);
    base_ptr[base + 2] = static_cast<uint32_t>(idx) - 32 + trailingzeroes(bits);
    bits = bits & (bits - 1);
    base_ptr[base + 3] = static_cast<uint32_t>(idx) - 32 + trailingzeroes(bits);
    bits = bits & (bits - 1);
    base_ptr[base + 4] = static_cast<uint32_t>(idx) - 32 + trailingzeroes(bits);
    bits = bits & (bits - 1);
    base_ptr[base + 5] = static_cast<uint32_t>(idx) - 32 + trailingzeroes(bits);
    bits = bits & (bits - 1);
    base_ptr[base + 6] = static_cast<uint32_t>(idx) - 32 + trailingzeroes(bits);
    bits = bits & (bits - 1);
    base_ptr[base + 7] = static_cast<uint32_t>(idx) - 32 + trailingzeroes(bits);
    bits = bits & (bits - 1);
    base += 8;
  }
  base = next_base;
...
really_inline uint32_t finalize_structurals(
    uint32_t structurals, uint32_t whitespace, uint32_t quote_mask,
    uint32_t quote_bits, uint32_t &prev_iter_ends_pseudo_pred) {
  ...
  uint32_t pseudo_pred = structurals | whitespace;

  uint32_t shifted_pseudo_pred =
      (pseudo_pred << 1) | prev_iter_ends_pseudo_pred;
  prev_iter_ends_pseudo_pred = pseudo_pred >> 31;
  uint32_t pseudo_structurals =
...
/*never_inline*/ bool find_structural_bits(const uint8_t *buf, size_t len,
  ...
  uint32_t prev_iter_ends_odd_backslash = 0ULL;
  ...
  uint32_t prev_iter_ends_pseudo_pred = 1ULL;
  ...
  uint32_t structurals = 0;

  size_t lenminus32 = len < 32 ? 0 : len - 32;
  size_t idx = 0;
  uint32_t error_mask = 0; // for unescaped characters within strings (ASCII code points < 0x20)

  for (; idx < lenminus32; idx += 32) {
#ifndef _MSC_VER
    __builtin_prefetch(buf + idx + 32);
#endif
    ...
    uint32_t odd_ends = find_odd_backslash_sequences(
        in, prev_iter_ends_odd_backslash);

    // detect insides of quote pairs ("quote_mask") and also our quote_bits
    // themselves
    uint32_t quote_bits;
    uint32_t quote_mask = find_quote_mask_and_bits(
